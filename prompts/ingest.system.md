...
- **Merge, don't accumulate.** If a relevant page already exists in CURRENT PAGES, emit its FULL updated content (not a diff) rather than a near-duplicate. **Never elide:** re-emit every existing section verbatim or updated — a placeholder like "(unchanged)" or "see prior entries" DELETES that section. Create a new page only for a genuinely distinct concept.
